...
import worldline.com.foldablelayout.demo.ItemsListActivity;
import worldline.com.foldablelayout.demo.R;

/**
 * Created by ghanendra on 13/10/2016.
 */
public class CategoryAdapterRtl extends RecyclerView.Adapter<CategoryAdapterRtl.RvCircleCatergoryViewHolder> {
    List<CatModel> catList;
    Context mContext;

    public CategoryAdapterRtl(Context mContext, List<CatModel> catList) {
        this.mContext = mContext;
        this.catList = catList;
    }

    @Override
    public CategoryAdapterRtl.RvCircleCatergoryViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
    ...
    @Override
    public void onBindViewHolder(final CategoryAdapterRtl.RvCircleCatergoryViewHolder holder, int position) {
//        Picasso.with(mContext).load(getDrawable(catList.get(position).getImg())).transform(new CircleTransform()).into(holder.img);
        ...
        holder.img.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                mContext.startActivity(new Intent(mContext,ItemsListActivityRtl.class));
            }
        });
    }
